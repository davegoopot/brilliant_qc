...
def test_probability_changes():
    assert (
        change_phase(ONE_KET).probability_measure_zero()
        == ONE_KET.probability_measure_zero()
    )
    assert (
        change_phase(ONE_KET).probability_measure_one()
        == ONE_KET.probability_measure_one()
    )


def test_multiple_gates():
    assert hadamard(change_phase(qc_not(ZERO_KET))) == Qubit(
